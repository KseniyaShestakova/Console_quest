...
class NodeParser:
    ...
    def parse_game(self, code_string, initial):
        if len(code_string) != 1:
            raise Exception('Syntax error in the line: ' + initial)
        self.action_lst.append(Action(self.action_game, 0, 0))

    def parse_score(self, code_string, initial):
        if len(code_string) != 2:
            raise Exception('Syntax error in the line: ' + initial)
        self.action_lst.append(Action(self.action_add_score, self.parameters[code_string[1]], 0))
    ...
    def parse_node(self):
        ...
        for code_string in code_strings:
            ...
            if code_string[0] == 'game':
                self.parse_game(code_string, initial)
            if code_string[0] == 'score':
                self.parse_score(code_string, initial)
